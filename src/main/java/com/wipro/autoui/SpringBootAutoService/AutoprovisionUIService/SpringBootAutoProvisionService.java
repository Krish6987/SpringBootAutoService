	...
	public int installTool(@PathVariable String tool, @PathVariable String ip_address, @PathVariable String password) {
		...
		if(errorCode == 0){
			if(tool.equals("sonarqube")){
				processBuilder.command("bash", "-c", "ansible-playbook /usr/src/app/playbooks/psql.yml -i "+ip_address+", -e 'target="+ip_address+"' && ansible-playbook /usr/src/app/playbooks/"+tool+".yml -i "+ip_address+", -e 'target="+ip_address+"' --key-file /usr/src/app/root/.ssh/id_rsa");
			}
			else{
